VERSION 5.00
Begin VB.UserControl AxLabelPlusNW 
   AutoRedraw      =   -1  'True
   ClientHeight    =   1125
   ClientLeft      =   0
   ClientTop       =   0
   ...
   FillStyle       =   0  'Solid
   PropertyPages   =   "axLabelPlusNW.ctx":0000
   ScaleHeight     =   75
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   282
   ToolboxBitmap   =   "axLabelPlusNW.ctx":000F
   ...
   End
End
Attribute VB_Name = "AxLabelPlusNW"
...
''Special thanks to: All members of the VB6 Latin group (www.leandroacierto.com/foro), vbforums.com and activevb.net
''-----------------------------------------------
'Modded Name: AxLabelPlusNW
'Editor:  David Rojas A. [AxioUK]
'LastUpdate: 18/10/2022
'Version: 1.6.16 -----------------------------
'- Removed features incompatible with non-windowless mode.
'- Changed Property BackColorPress to ChangeColorOnClick and use combinations from ChangeColorOnMouseOver.
'- Minor Bugfixes/improves
'Version: 1.6.15 -----------------------------
'- Non-WindowLess version to avoid GDI+ bug in graphical containers.
'- Added more combinations to property ChangeColorOnMouseOver
'- Adapted code to Non-WindowLess mode
'- Minor Bugfixes/improves
'Version: 1.6.14 -----------------------------
...
'- Added effect eIncreaseOpacity to picture on mouse over the contol
'- Added effect eAlternateGrayColor to picture on mouse over the contol
'Version: 1.6.12 -----------------------------
...
'- Minor bugfixes
'Version: 1.6.3 ------------------------------
'- Added more combinations and property ChangeColorOnMouseOver renamed to ChangeOnMouseOver
...
Private Declare Function GdipLoadImageFromStream Lib "GdiPlus.dll" (ByVal Stream As IUnknown, ByRef Image As Long) As Long
...
Private Declare Function GdipSetImageAttributesColorMatrix Lib "GdiPlus.dll" (ByVal imageattr As Long, ByVal ColorAdjust As Long, ByVal EnableFlag As Boolean, ByRef MatrixColor As COLORMATRIX, ByRef MatrixGray As COLORMATRIX, ByVal flags As Long) As Long
...
Private Declare Function GdipGetImageGraphicsContext Lib "GdiPlus.dll" (ByVal Image As Long, hGraphics As Long) As Long
...
Private Declare Function GdipCreateFontFamilyFromName Lib "GdiPlus.dll" (ByVal Name As Long, ByVal fontCollection As Long, fontFamily As Long) As Long
Private Declare Function GdipDeleteFontFamily Lib "GdiPlus.dll" (ByVal fontFamily As Long) As Long
...
Private Declare Function GdipCreateStringFormat Lib "GdiPlus.dll" (ByVal formatAttributes As Long, ByVal Language As Integer, StringFormat As Long) As Long
...
Private Declare Function GdipSetStringFormatAlign Lib "GdiPlus.dll" (ByVal StringFormat As Long, ByVal Align As eStringAlignment) As Long
...
Private Declare Function GdipCreateEffect Lib "GdiPlus.dll" (ByVal dwCid1 As Long, ByVal dwCid2 As Long, ByVal dwCid3 As Long, ByVal dwCid4 As Long, ByRef Effect As Long) As Long
Private Declare Function GdipSetEffectParameters Lib "GdiPlus.dll" (ByVal Effect As Long, ByRef params As Any, ByVal Size As Long) As Long
Private Declare Function GdipDeleteEffect Lib "GdiPlus.dll" (ByVal Effect As Long) As Long
Private Declare Function GdipDrawImageFX Lib "GdiPlus.dll" (ByVal graphics As Long, ByVal Image As Long, ByRef Source As RECTF, ByVal xForm As Long, ByVal Effect As Long, ByVal imageAttributes As Long, ByVal srcUnit As Long) As Long
...
Public Enum eCaptionAlignmentV
    cTop
    cMiddle
    cBottom
End Enum
...
Const m_def_eCaptionAlignmentH = 0
Const m_def_eCaptionAlignmentV = 0
...
Const m_def_ChangeColorOnClick = 0
...
Dim m_ColorOnClick As OLE_COLOR
Dim m_ColorOnClickOpacity As Integer
...
Dim m_eCaptionAlignmentH As eCaptionAlignmentH
Dim m_eCaptionAlignmentV As eCaptionAlignmentV
...
Dim m_ChangeColorOnClick As eChangeOnMouse
...
Dim m_IconAlignmentH  As eCaptionAlignmentH
Dim m_IconAlignmentV  As eCaptionAlignmentV
Dim m_IconOpacity     As Integer

Dim m_Bitmap As Long
...
Dim m_PicEffect     As eChangeOpacityEffect
Dim oldOpacityValue As Integer
...
Public Sub Draw(ByVal hDC As Long, ByVal PosX As Long, PosY As Long)
...
'*2
    If m_Gradient Then
        ...
        If m_Clicked Then
          If m_ChangeColorOnClick = eChangeBackColor Then
        ...
        Else
          If m_MouseOver Then
            If m_ChangeOnMouseOver = eChangeBackColor Then
              GdipCreateLineBrushFromRectWithAngleI RECTL, ConvertColor(m_ColorOnMouseOver, m_ColorOnMouseOverOpacity), _
    ...
    Else
        If m_Clicked Then
          If m_ChangeColorOnClick = eChangeBackColor Then
            GdipCreateSolidFill ConvertColor(m_ColorOnClick, m_ColorOnClickOpacity), hBrush
            GoTo Paso0
          Else
            GoTo NoClicked0
          End If
...
'*2
    If m_Border And BorderWidth > 0 Then
      If m_Clicked Then
        Select Case m_ChangeColorOnClick
          Case Is = eChangeBorderColor, eChangeCaptionBorder, eChangeIconBorder
              GdipCreatePen1 ConvertColor(m_ColorOnClick, m_ColorOnClickOpacity), BorderWidth, UnitPixel, hPen
          Case Else
  ...
  If m_CrossVisible Then
    Select Case m_CrossPosition
      ...
      Case Is = cTopLeft
          ...
          Else
            YCrossPos = BorderWidth + 10
            XCrossPos = (BorderWidth + 10) + IIf(m_HotLine, m_HotLineWidth, 0)
          End If
...
Public Function IsMouseOverExtender() As Boolean
    Dim PT As POINTAPI
    GetCursorPos PT
    IsMouseOverExtender = (WindowFromPoint(PT.X, PT.Y) = UserControl.hwnd)
    'Debug.Print "IsMouseOverExtender:" & IsMouseOverExtender
End Function
...
Public Function LoadImage(SrcImg, Optional ByVal UseCache As Boolean, Optional ByVal DrawProgress As Boolean = True) As Boolean
    ...
    Select Case VarType(SrcImg)
        Case vbString
            ...
            If Left$(LCase(SrcImg), 5) = "data:" Then
                Base64Decode Split(SrcImg, ",")(1), m_PictureArr
                LoadImage = LoadImageFromArray(m_PictureArr, m_Bitmap)
            Else
                m_DrawProgress = DrawProgress
                UserControl.CancelAsyncRead "URL"
                Err.Clear
                Call AsyncRead(SrcImg, vbAsyncTypeByteArray, "URL", IIf(UseCache, 0, vbAsyncReadForceUpdate))
                LoadImage = CBool(Err.Number = 0&)
                Exit Function
            End If
        Case vbLong
            ...
            If GetObjectType(SrcImg) = OBJ_BITMAP Then
                LoadImage = GdipCreateBitmapFromHBITMAP(SrcImg, 0, m_Bitmap) = 0&
            Else
                LoadImage = GdipCreateBitmapFromHICON(SrcImg, m_Bitmap) = 0&
            End If
            
        Case vbDataObject
            LoadImage = GdipLoadImageFromStream(SrcImg, m_Bitmap) = 0&
            
        Case (vbArray Or vbByte)
            m_PictureArr() = SrcImg
            If LoadImageFromArray(m_PictureArr, m_Bitmap) Then
                LoadImage = True
            Else
                Erase m_PictureArr
            End If
    ...
    If m_Bitmap Then
        GdipGetImageWidth m_Bitmap, m_PictureRealWidth
        GdipGetImageHeight m_Bitmap, m_PictureRealHeight
...
Public Sub Refresh()
    Dim X As Long, Y As Long
    UserControl.Cls
    RaiseEvent PrePaint(UserControl.hDC, X, Y)
    Draw UserControl.hDC, X, Y
...
Private Function CreateBrushTexture(hGraphics As Long, hPath As Long, ByVal X As Long, ByVal Y As Long, ByVal Width As Long, ByVal Height As Long) As Long
    ...
    If m_Bitmap <> 0 Then 'LoadImageFromArray(m_PictureArr, hImage)
        ...
        If m_PictureShadow = True And m_ShadowSize > 0 And m_ShadowColorOpacity > 0 Then
            ...
            ShadowSize = m_ShadowSize * nScale
            hPictureShadow = CreateBlurShadowImage(m_Bitmap, m_ShadowColor, ShadowSize, 0, 0, m_PictureRealWidth, m_PictureRealHeight)
...
NextEfCap1:
            ...
            If m_IconCharCode Then
                ...
                'IconChar     'm_ColorOnClick, m_ColorOnClickOpacity
                If m_Clicked Then
                  Select Case m_ChangeColorOnClick
                    Case Is = eChangeCaptionIcon, eChangeIconOnly, eChangeIconBorder
                      GdipCreateSolidFill ConvertColor(m_ColorOnClick, IIf(ForShadow, m_ShadowColorOpacity, m_IconOpacity)), hBrush
                      GoTo NextEfIcon
                    Case Else
                      GoTo NoEfIcon
                  End Select
...
Dim lHwnd As Long
    ...
    For Each Ctrl In Frm.Controls
        With Ctrl
           If TypeOf Ctrl Is AxLabelPlusNW Then
...
If m_PicEffect = eIncreaseOpacity Then
    If PictureOpacity >= 100 Then
      tmrEffect.Enabled = False
    Else
      PictureOpacity = PictureOpacity + 2
    End If
End If

If Not IsMouseOverExtender Then tmrEffect.Enabled = False
...
If Not IsMouseOverExtender Then
...
Private Sub UserControl_AsyncReadComplete(AsyncProp As AsyncProperty)
    ...
    If LoadImage(AsyncProp.Value) Then
...
Private Sub UserControl_Initialize()
    ...
    Call GdiplusStartup(hGdipToken, GdipStartupInput, ByVal 0)
...
Private Sub UserControl_InitProperties()
    ...
    m_ColorOnClick = m_def_BackColorP
    m_ColorOnClickOpacity = m_def_BackColorPOpacity
    ...
    m_eCaptionAlignmentH = m_def_eCaptionAlignmentH
    m_eCaptionAlignmentV = m_def_eCaptionAlignmentV
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        m_ColorOnClick = .ReadProperty("BackColorPress", m_def_BackColorP)
        m_ColorOnClickOpacity = .ReadProperty("BackColorPressOpacity", m_def_BackColorPOpacity)
        ...
        m_eCaptionAlignmentH = .ReadProperty("CaptionAlignmentH", m_def_eCaptionAlignmentH)
        m_eCaptionAlignmentV = .ReadProperty("CaptionAlignmentV", m_def_eCaptionAlignmentV)
...
On Error Resume Next
    If m_Bitmap Then GdipDisposeImage m_Bitmap
    If m_PictureBrush Then GdipDeleteBrush m_PictureBrush: m_PictureBrush = 0
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    With PropBag
        Call .WriteProperty("BackGroundColor", m_BackGroundColor, UserControl.Ambient.BackColor)
        ...
        Call .WriteProperty("BackColorPress", m_ColorOnClick, m_def_BackColorP)
        Call .WriteProperty("BackColorPressOpacity", m_ColorOnClickOpacity, m_def_BackColorPOpacity)
        ...
        Call .WriteProperty("CaptionAlignmentH", m_eCaptionAlignmentH, m_def_eCaptionAlignmentH)
        Call .WriteProperty("CaptionAlignmentV", m_eCaptionAlignmentV, m_def_eCaptionAlignmentV)
...
End Property
'<<------------------------------------------------------->>
Public Property Get ColorOnClick() As OLE_COLOR
    ColorOnClick = m_ColorOnClick
End Property

Public Property Let ColorOnClick(ByVal New_ColorOnClick As OLE_COLOR)
    m_ColorOnClick = New_ColorOnClick
    PropertyChanged "ColorOnClick"
    Refresh
End Property

Public Property Get ColorOnClickOpacity() As Integer
    ColorOnClickOpacity = m_ColorOnClickOpacity
End Property

Public Property Let ColorOnClickOpacity(ByVal New_ColorOnClickPOpacity As Integer)
    m_ColorOnClickOpacity = New_ColorOnClickPOpacity
    SafeRange m_ColorOnClickOpacity, 0, 100
    PropertyChanged "ColorOnClickOpacity"
    CreateShadow
    Refresh
End Property
'------------------------------------------
...
Public Property Let CaptionAlignmentH(ByVal New_eCaptionAlignmentH As eCaptionAlignmentH)
    m_eCaptionAlignmentH = New_eCaptionAlignmentH
    PropertyChanged "CaptionAlignmentH"
    bRecreateShadowCaption = True
    Refresh
End Property

Public Property Get CaptionAlignmentV() As eCaptionAlignmentV
    CaptionAlignmentV = m_eCaptionAlignmentV
End Property

Public Property Let CaptionAlignmentV(ByVal New_eCaptionAlignmentV As eCaptionAlignmentV)
    m_eCaptionAlignmentV = New_eCaptionAlignmentV
...
Public Property Get ChangeColorOnClick() As eChangeOnMouse
    ChangeColorOnClick = m_ChangeColorOnClick
End Property

Public Property Let ChangeColorOnClick(ByVal New_Change As eChangeOnMouse)
...
Public Property Get IconAlignmentH() As eCaptionAlignmentH
    IconAlignmentH = m_IconAlignmentH
End Property

Public Property Let IconAlignmentH(ByVal New_IconAlignmentH As eCaptionAlignmentH)
...
Public Property Get IconAlignmentV() As eCaptionAlignmentV
    IconAlignmentV = m_IconAlignmentV
End Property

Public Property Let IconAlignmentV(ByVal New_IconAlignmentV As eCaptionAlignmentV)
...
Public Property Get IconCharCode() As String
    IconCharCode = "&H" & VBA.Hex(m_IconCharCode)
End Property
